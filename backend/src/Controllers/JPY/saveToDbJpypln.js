...
app.post("/api/saveJpy", async (req, res) => {
  try {
    var url =
      "https://www.alphavantage.co/query?function=FX_DAILY&from_symbol=JPY&to_symbol=PLN&outputsize=full&apikey=4AOCSQKH2KXMMI46";
        ...
        if (err) {
          ...
          currencyModel
            ...
            .then(async (result) => {
              ...
              console.log("Add ror JPY/PLN");
            });
        }
      }
    );
    currencyModel.find({ currency: "JPY" }).then(async (result) => {
      res.send("Dodano poprawnie JPY");
    });
  } catch (error) {
