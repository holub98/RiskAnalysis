...
app.post("/api/saveGbp", async (req, res) => {
  try {
    var url =
      "https://www.alphavantage.co/query?function=FX_DAILY&from_symbol=GBP&to_symbol=PLN&outputsize=full&apikey=4AOCSQKH2KXMMI46";
        ...
        if (err) {
          ...
          currencyModel
            ...
            .then(async (result) => {
              ...
              console.log("Add ror GBP/PLN");
            });
        }
      }
    );
    currencyModel.find({ currency: "GBP" }).then(async (result) => {
      res.send("Dodano poprawnie GBP");
    });
  } catch (error) {
